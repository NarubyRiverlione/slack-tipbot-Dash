...
var TipBot = function (RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS, WALLET_PASS) {
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
     ...
     //   PRICETICKER_CHANNEL: null,
        PRICETICKER_TIMER: 10 // seconds for the momen
    });
...
function tellPrice(self, reply, currency) {
    self.getPriceRates(function (err, rates) {
        ...
        if (!rate) {
            ...
            reply["text"] = self.text["PriceBase"] + rate.toPrecision(3) + " " + currency + self.text["PriceInfoFrom"];
        }
        self.slack.say(reply);
    });
    return;
...
TipBot.prototype.onMessage = function (channel, member, message) {
                ...
                if (fromCurrency === "dash") {
                    ...
                    self.normalizeValue(amount[1], toCurrency, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply["text"] = amount[1] + " " + fromCurrency + " = " + newValue.toPrecision(3) + "  " + toCurrency
    ...
    if (message.match(/\bprice\b/i)) {
        ...
        if (currency) {
            ...
            tellPrice(self, reply, currencies);
    ...
    if (message.match(/\bpriceticker\b/i)) {
        ...
        if (message.match(/\brepeat\b/i)) {
            reply["text"] = "I will tell the prices every " + self.OPTIONS.PRICETICKER_TIMER;
            self.slack.say(reply);

            repeat(self.OPTIONS.PRICETICKER_TIMER * 1000,
                function () {
