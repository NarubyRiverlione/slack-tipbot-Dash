...
TipBot.prototype.tellPrice = function (currency, cb) {
    ...
    self.getPriceRates(function (err, rates) {
        ...
        if (!rate) {
            cb(helpTexts.UnsupportedCurrency, null);
        } else {
            cb(null, helpTexts.PriceBase + rate.toPrecision(4) + ' ' + currency);
        }
    });
};
    ...
    sun.checkThreshold(self.OPTIONS.SUN_THRESHOLD, self.sunUser,
        function (err, reviecedUsers, sunraySize) {
            ...
            if (sunraySize !== null && reviecedUsers !== null) {
                ...
                let reply = {
                    channel: self.OPTIONS.MAIN_CHANNEL.id,
                    text: helpTexts.SunRay + '\n' +
                    helpTexts.SunExplain + '\n' +
                    helpTexts.SunRay + '\n'
                };
                self.slack.say(reply);
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
        ...
        if (message.match(/\bwithdraw\b/i)) {
            ...
            self.normalizeValue(amount[1], amount[2], user, function (value, rate, originalCurrency, originalValue, valueText) {
                if (rate === false) {
                    ...
                    privateReply.text = helpTexts.WithdrawQuestion[0] + valueText +
                        helpTexts.WithdrawQuestion[1] + address +
                        helpTexts.WithdrawQuestion[2];
                    self.slack.say(privateReply);
                    ...
                    self.triggers.push(new Trigger(
                        ...
                        function (channel, message, _user) {
                            ...
                            if (channel.id === privateReply.channel && _user.id === user.id && message.match(/(OK|yes|fine|sure)/i)) {
                                ...
                                trigger.destroy();
                                return true;
                            } else if (channel.id === privateReply.channel && _user.id === user.id && message.match(/(no|cancel|abort)/i)) {
                                trigger.destroy();
        ...
        if (helpTexts.NoCommandFound[randomRespons] === undefined) {
