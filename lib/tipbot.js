...
var TipBot = function (RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS, WALLET_PASS) {
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        RAIN_SEND_THROTTLE: 1250, // ms wait between raindrops to fall (prevent slack spam protection)
        ...
        // RAIN_TIMER: 15 // random next x  seconds (rain timer sets and shows time in minutes)
...
TipBot.prototype.updateUserFromMember = function (member, updateRegex) {
    ...
    // warn admins that new users has arrived, only when not initializing the tipbot (also check if new users are detected)
    if (self.OPTIONS.MODERATOR_CHANNEL !== undefined && !self.initializing) {
        var newUserMsg = {
            channel: self.OPTIONS.MODERATOR_CHANNEL.id,
        ...
        debug("User update: Mods are not warned because : init=" + self.initializing + ", or mod channel=" + self.OPTIONS.MODERATOR_CHANNEL);
    }
};
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
        ...
        //   * RAIN (reward to all online users)
        if (message.match(/\brain\b/i)) {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\btimer\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        self.OPTIONS.RAIN_TIMER = amount[1] * 60; //  sec
                    }
                }
            ...
            if (self.OPTIONS.RAIN_TIMER !== undefined) {
                ...
                reply["text"] = self.text["RainTimer"] + self.OPTIONS.RAIN_TIMER / 60 + self.text["RainTimerUnit"];
                self.slack.say(reply);
            } else if (user.is_admin) {
        ...
        if (message.match(/\bsun\b/i)) {
            ...
            if (user.is_admin) {
                // show tippers
                if (message.match(/\bshow\b/i)) {
                    Tipper.find(function (err, allTippers) {
                        if (err) {
                            debug("ERROR reading db:" + err);
                            return;
                        }
                        reply["text"] = "*This users have tipped* \n";
                        // list all tippers
                        allTippers.forEach(function (tipper) {
                            reply["text"] += tipper.name + "(" + tipper.id + ") has tipped " + tipper.tipCount + " times.\n";
                        });
                        debug(reply["text"]);
                        self.slack.say(reply);
                    });
                ...
                if (message.match(/\breset\b/i)) {
                    Tipper.remove(function (err) {
                        if (err) debug("ERROR database: removing all reccords");
                        else {
                            privateReply["text"] = "All tip count records are removed.";
                            debug(privateReply["text"] + " by " + user.name); // log removing
                            self.slack.say(privateReply);
                        }
                    });
                }
...
// it Raining Dash !
TipBot.prototype.rainNow = function (channel) {
    var self = this;
    if (self.RainUser === undefined || self.RainUser === null) {
    ...
    // get raindrop size
    getCurrentRainDropsSize(self, function (err, rainDropSize, amountOnlineUsers, onlineUsers) {
        // raindropSize is in satoshis
        var reply = { "channel": channel.id };
        if (err) {
