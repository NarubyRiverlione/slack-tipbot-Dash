...
function listOnlineUsers(self, cb) {
    ...
    self.slack.api.users.list({ "presence": 1 }, function (err, data) {
        if (err) {
            debug("ERROR cannot get list of online users: " + err);
            cb(err, null);
        }
        // TODO refactoring, _.map + _.filter ?        
        for (var userNumber in data.members) {
            var oneUser = data.members[userNumber];
            if (oneUser.presence === "active" && oneUser.is_bot === false && oneUser.name !== self.OPTIONS.RAIN_USERNAME) {
    ...
    listOnlineUsers(self, function (err, onlineUsers) {
        ...
        getRainBalance(self, user, function (err, rainBalance) {
            if (err) {
                ...
                cb(null, rainBalance / amountOnlineUsers, amountOnlineUsers, onlineUsers);
            }
        });
...
function rainNow(self, user, channel) {
    ...
    getCurrentRainDropsSize(self, user, function (err, rainDropSize, amountOnlineUsers, onlineUsers) {
        ...
        if (rainDropSize === 0) {
            ...
            self.slack.say(reply);
            debug("Raindrop = 0, nothing to give away.");
            return;
        }
        ...
        debug("===== ASYNC start rain =====");
        async.forEachSeries(_.keys(onlineUsers), function (oneUser, cb) {
            ...
            if (getUserWet) { // only continue if user object was found 
                debug("Let a raindrop of " + rainDropSize.toPrecision(4) + " dash fall on " + getUserWet.name + " (" + getUserWet.id + ")");
                // wait a second to prevent slack spam protection
                wait(self.OPTION.RAIN_SEND_THROTTLE, function () {
                    self.RainUser.send(channel.id, getUserWet, rainDropSize, cb); // callback needed to let async know everyhing is done
                });
            }
        },
            // function called when all async tasks are done
            function (err) {
                if (err) debug("ERROR during async rain: " + err);
                debug("===== ASYNC stop rain =====");
            });
    });
}
