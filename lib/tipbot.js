...
function getRainBalance(self, rainUserID, user, cb) {
    // get balance of Rain User
    user.getBalance(rainUserID, 6, function (err, rainBalance) {
        if (err) cb(err, null);
        return cb(null, rainBalance);
    });

}
// find the Slack Rain user
function getRainUser(self) {
    ...
    if (findRainUser === undefined || findRainUser.length !== 1) {
        debug(self.text["RainCannotFindRainAccount"] + self.OPTIONS.RAIN_USER + "'");
        return null;
    }
    return findRainUser[0];
}
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    if (self.OPTIONS.RAIN_THRESHOLD !== undefined) {
        var rainUser = getRainUser(self);
        if (rainUser === null) {
            // only check if rain threshold is set
            getRainBalance(self, rainUser.id, user, function (err, rainBalance) {
                if (!err) {
    ...
    if (message.match(/\brain\b/i)) {
        ...
        var rainUser = getRainUser(self);
        if (rainUser === null) {
            reply["text"] = self.text["RainCannotFindRainAccount"] + self.OPTIONS.RAIN_USER + "'";
            self.slack.say(reply);
            return;
        }
        ...
        self.slack.api.users.list({ "presence": 1 }, function (err, data) {
            ...
            amountOnlineUsers = Object.keys(onlineUsers).length;
            // rain drops = rain / online user
            var rainDrops = availableRain / amountOnlineUsers;
            reply["text"] = rainDrops.toPrecision(4) + self.text["RainPerUser"] + amountOnlineUsers + ")";
            self.slack.say(reply);

            if (message.match(/\bnow\b/i)) {
                // let it rain NOW
                for (var getWet in onlineUsers) {
                    debug(self.text["RainNow_1"] + getWet + " " + rainDrops.toPrecision(4)
                        + self.text["RainNow_2"] + rainUser.name);
                    //  findRainUser[0].send(channel.id, getWet, value);
                }
            }
        });
        return;
