...
TipBot.prototype.tellHelp = function () {
    ...
    if (this.OPTIONS.DEMAND === true) {
        helpText += " - *receive*\t\ttell the bot to request coins from to someone; _@tipbot receive 0.1 DASH from @someone_ \n" +
            " _aliases: demand, ask, deserve, get, give me, owes me_ \n" +
            "\n";
    }

    helpText += " - *currencies*\task the bot for a list of supported currencies; _@tipbot what currencies do you know?_ \n" +
        "\n" +
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    if (message.match(/(send|give|sent|tip)/i)) {
        ...
        if (userMatches.length == 0) {
            reply["text"] = "Hello there " + user.handle + ", I didn't catch the user you want to tip. You surely didn't want to tip yourself, did you ?";
            self.slack.say(reply);
            return;
            ...
            self.normalizeValue(amount[1], amount[2], function (value, rate, originalCurrency, originalValue, valueText) {
                if (rate === false) {
                    ...
                    // no minimum needed as there is not blockchain transaction ?
                    // if (value < MIN_TIP) {
                    ...
                    // Finally all checks are passed, send to tip
    ...
    if (message.match(/currencies/i)) {
        reply["text"] = "Price info from coinmarketcap \n" +
            ...
            "use these currency abbreviations/symbols in your message: *" + self.CURRENCIES.join(", ") + "* \n" +
            "And does it need saying: *DASH* is supported !";
        self.slack.say(reply);
        return;
    }
