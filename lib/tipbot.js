...
var TipBot = function (RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS, WALLET_PASS) {
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        PRICETICKER_TIMER: 60 // minutes for the moment
...
TipBot.prototype.showPriceTicker = function (priceChannel, all) {
    var self = this;
    ...
    for (var currency in priceList) {
        debug("Pricelist: show " + priceList[currency]);
...
function tellPrice(self, reply, currency) {
    self.getPriceRates(function (err, rates) {
        ...
        if (!rate) {
            ...
            reply["text"] = self.text["PriceBase"] + rate.toPrecision(3) + " " + currency;
        }
        self.slack.say(reply);
    });
    return;
...
TipBot.prototype.init = function (bot) {
    ...
    bot.api.users.list({}, function (err, data) {
        ...
        async.forEachLimit(data.members, 100, function (member, cb) {
            ...
            // get Rain User
            var findRainUser = _.filter(self.users, function (match) { return match.name.match(self.OPTIONS.RAIN_USER, "i"); });
            if (findRainUser === undefined || findRainUser.length !== 1) {
                debug("ERROR : " + self.text["RainCannotFindRainAccount_1"] + self.OPTIONS.RAIN_USER + self.text["RainCannotFindRainAccount_2"]);
            } else {
...
TipBot.prototype.onMessage = function (channel, member, message) {
                        ...
                        self.triggers.push(new Trigger(
                            ...
                            function (channel, message, _user) {
                                ...
                                if (channel.id === DMchannelID && _user.id == user.id && message.match(/(OK|yes|fine|sure)/i)) {
                                    ...
                                    user.withdraw(DMchannelID, value, address[0], self.WALLET_PASS);
                                    trigger.destroy();
    ...
    if (message.match(/\b(send|give|sent|tip)\b/i)) {
        ...
        if (userMatches.length == 0) {
            ...
            self.normalizeValue(amount[1], amount[2], function (value, rate) {
                if (rate === false) {
                    reply["text"] = user.handle + self.text["UnsupportedCurrency"];
                    self.slack.say(reply);
                } else if (!value) {
        ...
        if (currencies.length < 2) {
            ...
            if (amount === null) {
                ...
                if (fromCurrency === "dash") {
                    // Dash -> other
                    self.normalizeValue(amount[1], toCurrency, function (value, rate) {
                        if (rate === false) {
                            reply["text"] = user.handle + self.text["UnsupportedCurrency"];
                        } else if (!value) {
                            reply["text"] = user.handle + self.text["InvalidAmount"];
                            ...
                            reply["text"] = amount[1] + " " + fromCurrency + " = " + newValue.toPrecision(4) + "  " + toCurrency
                                + " ( 1.0 Dash = " + rate.toPrecision(3) + " " + toCurrency + " )";
                        }
                        self.slack.say(reply);
                    });
                } else if (toCurrency === "dash") {
                    // other -> dash
                    self.normalizeValue(amount[1], fromCurrency, function (value, rate) {
                        if (rate === false) {
                            reply["text"] = user.handle + self.text["UnsupportedCurrency"];
                        } else if (!value) {
                            reply["text"] = user.handle + self.text["InvalidAmount"];
                            ...
                            reply["text"] = amount[1] + " " + fromCurrency + " = " + blocktrail.toBTC(value) + "  " + toCurrency
                                + " ( 1.0 " + fromCurrency + " = " + (1.0 / rate).toPrecision(3) + " DASH)";
                        }
                        self.slack.say(reply);
                    });
                }
    ...
    if (message.match(/\bpriceticker\b/i)) {
        ...
        if (message.match(/\brepeat\b/i)) {
            ...
            repeat(self.OPTIONS.PRICETICKER_TIMER * 1000 * 60,
                function () {
                    self.showPriceTicker(tellChannel.id, false);
                },
                "priceticker",
                true);
            return;
        ...
        if (message.match(/\bshort\b/i)) {
            ...
            // show all currencies
            self.showPriceTicker(tellChannel.id, true);
        }
        return;
    }
    ...
    if (message.match(/\brain\b/i)) {
        ...
        if (self.RainUser === undefined || self.RainUser === null) {
            reply["text"] = self.text["RainCannotFindRainAccount_1"] + self.OPTIONS.RAIN_USER + self.text["RainCannotFindRainAccount_2"];
            self.slack.say(reply);
            return;
        }

        // get balance of Rain Account
        var availableRain = 0.0;
        getRainBalance(self, user, function (err, rainBalance) {
            if (err) {
                reply["text"] = self.text["RainCannotFindRainBalance"] + self.OPTIONS.RAIN_USER;
                self.slack.say(reply);
                return;
            } else {
        ...
        if (!user.is_admin) {
            reply["text"] = self.text["RainAdminOnly"];
            self.slack.say(reply);
            return;
        }
        ...
        if (message.match(/\bthreshold\b/i)) {
            ...
            if (amount !== null) {
                ...
                reply["text"] = self.text["RainThresholdNew"] + "\n";
            }
            ...
            if (self.OPTIONS.RAIN_THRESHOLD !== undefined) {
                ...
                reply["text"] = self.text["RainThresholdNotSet"];
                self.slack.say(reply);
            }
            return;
        }
        ...
        if (message.match(/\btimer\b/i)) {
            ...
            if (amount !== null) {
                ...
                reply["text"] = self.text["RainTimerNew"] + "\n";
            }

            if (self.OPTIONS.RAIN_TIMER !== undefined()) {
                // show the current timer
                if (reply["text"] === undefined) reply["text"] = "";
                reply["text"] += self.text["RainTimer"] + self.OPTIONS.RAIN_TIMER / 1000 / 60
                    + self.text["RainTimerUnit"];
                self.slack.say(reply);
            } else {
                // no timer set yet
                reply["text"] = self.text["RainTimerNotSet"];
                self.slack.say(reply);
            }
            return;
        }

        // show rain drop size (available rain / current online users)
        getRainDropsSize(self, user, function (err, rainDropSize, amountOnlineUsers) {
            if (!err) {
                reply["text"] = rainDropSize.toPrecision(4) + self.text["RainPerUser"] + amountOnlineUsers + ")";
                self.slack.say(reply);
            }
        });
...
function listOnlineUsers(self, cb) {
    ...
    self.slack.api.users.list({ "presence": 1 }, function (err, data) {
        ...
        for (var userNumber in data.members) {
            var oneUser = data.members[userNumber];
            if (oneUser.presence === "active" && oneUser.is_bot === false) {
                //debug(oneUser.presence + ": " + oneUser.name + " = " + oneUser.id);
...
function getRainBalance(self, user, cb) {
    if (self.RainUser === undefined || self.RainUser === null) {
        debug("ERROR cannot get rain balance as Rain User is unknow !");
...
//  amount dash / online user = size of rain drop
function getRainDropsSize(self, user, cb) {
    listOnlineUsers(self, function (err, onlineUsers) {
        ...
        getRainBalance(self, user, function (err, rainBalance) {
            if (err) {
                debug("ERROR cannnot calculated size of a rain drop if available rain is unknow !");
                cb("RainDropSizeUnknown", null, null);
            } else {
                cb(null, rainBalance / amountOnlineUsers, amountOnlineUsers);
            }
        });
...
// it Raining Dash !
function rainNow(self, user) {
    var onlineUsers = listOnlineUsers(self);
    getRainDropsSize(self, user, function (err, rainDropSize) {
        if (err) {
            debug("ERROR cannot make it rain as rain drop size is unknown !");
            return;
        } else {
            // let it rain NOW
            for (var getWet in onlineUsers) {
                debug(self.text["RainNow_1"] + getWet + " " + rainDropSize.toPrecision(4)
                    + self.text["RainNow_2"] + self.RainUser.name);
                //  findRainUser[0].send(channel.id, getWet, rainDropSize);
            }
        }
    });
