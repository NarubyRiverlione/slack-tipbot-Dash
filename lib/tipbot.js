...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        TX_FEE: dash.toDuff(0.0001),     // TX fee, used in withdrawing, in satochi
        WALLET_PASSW: null,
        ...
        SUN_THRESHOLD: dash.toDuff(5) // satoshis
    });
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        self.OPTIONS.SUN_THRESHOLD = dash.toDuff(amount[1]);
                        // threshold changed => check balance now
                        self.checkForSun(reply);
                    }
                    //show threshold
                    if (self.OPTIONS.SUN_THRESHOLD !== undefined) {
                        reply.text = texts.SunThreshold1 + dash.toDash(self.OPTIONS.SUN_THRESHOLD) + ' Dash \n';
        ...
        if (message.match(/\bquiz\b/i)) {
            ...
            if (message.match(/\banswer\b/i)) {
                ...
                quiz.checkAnswer(answer, function (err, done, correct) {
                    if (err) { debug(err); return; }
                    if (correct) {
                        // correct answer
                       ...
                       // self.slack.say(reply);
                        if (done) {
                            reply.text += texts.QuizDone;
                            // TODO show scores
                            return;
                        } else {
                            //TODO DRY first question in start
                            // show next question
                            quiz.askQuestion(function (err, questionSentence) {
                                if (err) {
                                    debug('ERROR cannot tell a quiz question: ' + err);
