...
let TipBot = function (RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    let self = this;
...
TipBot.prototype.addUser = function (user, updateRegex) {
    ...
    if (self.OPTIONS.MODERATOR_CHANNEL !== undefined && !self.initializing) {
        let newUserMsg = {
            ...
            text: self.Text.WarningNewUser1 +
            user.name +
            self.Text.WarningNewUser2
        };
...
TipBot.prototype.updateUserFromMember = function (member, updateRegex) {
    ...
    if (member.deleted || member.is_bot) {
        ...
        if (self.OPTIONS.MODERATOR_CHANNEL !== undefined && !self.initializing) {
            let userLeftMsg = {
                channel: self.OPTIONS.MODERATOR_CHANNEL.id,
                text: self.Text.WarnUserLeft1 +
                member.name +
                self.Text.WarnUserLeft2
            };
    ...
    bot.api.users.list({}, function (err, data) {
        ...
        async.forEachLimit(data.members, 100, function (member, cb) {
            ...
            if (findSunUser === undefined || findSunUser.length !== 1) {
                debug('ERROR Init: ' + self.Text.SunCannotFindSunAccount1 +
                    self.OPTIONS.SUN_USERNAME +
                    self.Text.SunCannotFindSunAccount2);
            } else {
...
function getSunBalance(self, cb) {
    if (self.sunUser === undefined || self.sunUser === null) {
        debug('ERROR Sun: ' + self.Text.SunCannotFindSunAccount1);
...
TipBot.prototype.showRandomHelp = function () {
    ...
    let helpMsg = {
        ...
        text: self.Text.HelpRandom1 + '\n' +
        '```' + helpTxt + '``` \n\n' +
        self.Text.HelpRandom2
    };
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
                    ...
                    self.triggers.push(new Trigger(
                        ...
                        function (channel, message, _user) {
                            ...
                            if (channel.id === privateReply.channel && _user.id === user.id && message.match(/(OK|yes|fine|sure)/i)) {
                                user.withdraw(value, address[0], self.OPTIONS.WALLET_PASS, function (err, response) {
                                    if (err) {
        ...
        if (message.match(/\bpriceticker\b/i)) {
            ...
            if (message.match(/\bshort\b/i)) {
                ...
                privateReply.text = self.Text.LocationOfPriceList1 + self.OPTIONS.PRICETICKER_CHANNEL.name + self.Text.LocationOfPriceList2;
                self.slack.say(privateReply);
            }
            return;
        }
        ...
        if (message.match(/\bsun\b/i)) {
            ...
            if (self.sunUser === undefined || self.sunUser === null) {
                reply.text = self.Text.SunCannotFindSunAccount1 + self.OPTIONS.SUN_USERNAME + self.Text.SunCannotFindSunAccount2;
            ...
            getSunBalance(self, function (err, sunBalance) {
                if (err) {
                    ...
                    reply.text += '\n' + self.Text.SunReqDonation1 + self.OPTIONS.SUN_USERNAME + '_';
                    reply.text += '\n' + self.Text.SunReqDonation2 + self.OPTIONS.SUN_USERNAME + self.Text.SunReqDonation3;
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (self.OPTIONS.SUN_THRESHOLD !== undefined) {
                        reply.text = self.Text.SunThreshold1 + blocktrail.toBTC(self.OPTIONS.SUN_THRESHOLD) + ' Dash \n';
                        reply.text += self.Text.SunThreshold2;
                        self.slack.say(reply);
                    } else {
