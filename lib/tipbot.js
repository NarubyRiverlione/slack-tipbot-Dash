...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
  ...
  self.HighBalanceWarningMark = dash.toDuff(2.0);
  ...
  self.OPTIONS = _.defaults(OPTIONS, {
    ...
    TX_FEE: dash.toDuff(0.0001), // TX fee, used in withdrawing, in Duffs
    WALLET_PASSW: null,
    ...
    SUN_DEFAULT_THRESHOLD: dash.toDuff(5) // duff
  });
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
  ...
  if (inputValue === 'all' && user !== undefined) {
    user.getBalance(user.id, 6, function (err, balance) {
      ...
      let value = dash.toDuff(balance);
      debug('Log: using ALL balance of ' + user.name + ' = ' + balance);
      cb(value, null, null, null, dash.toDash(value) + ' DASH');
    });
  ...
  if (currency !== 'DASH') {
    ...
    self.getPriceRates(function (err, rates) {
      ...
      if (!rate) {
        ...
        let text = value.toFixed(2) + ' ' + currency + ' ' +
          '(' + dash.toDash(newValue) + ' DASH at ' + rate.toFixed(2) + ' ' + currency + ' / DASH)';
    ...
    return cb(value, null, null, null, dash.toDash(value) + ' DASH');
  }
};
                ...
                self.getDirectMessageChannelID(null, oneUser.id, function (err, DMchannelRecievingUser) {
                  if (err === null) {
                    ...
                    let recievingUserMessage = {
                      ...
                      'text': texts.SunRecieved + dash.toDash(sunraySize) + ' dash'
                    };
        ...
        if (fromCurrency === 'dash') {
          ...
          self.normalizeValue(amount[1], fromCurrency, user, function (value, rate) {
            if (rate === false) {
              ...
              reply.text = amount[1] + ' ' + fromCurrency + ' = ' + dash.toDash(value) + '  ' + toCurrency +
      ...
      if (user.is_admin) {
        ...
        if (message.match(/\bthreshold\b/i)) {
          ...
          if (amount !== null) {
            ...
            sun.saveThreshold(dash.toDuff(amount[1]),
              function (err) {
                if (err) {
                  debug(err); return;
                } else {
