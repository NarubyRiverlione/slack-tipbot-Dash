...
const BLACKLIST_CURRENCIES = ['NXS'];
...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
  ...
  self.HighBalanceWarningMark = coin.toDuff(2.0);
  ...
  self.OPTIONS = _.defaults(OPTIONS, {
    ...
    TX_FEE: coin.toDuff(0.0001), // TX fee, used in withdrawing, in Duffs
    WALLET_PASSW: null,
    ...
    SUN_DEFAULT_THRESHOLD: coin.toDuff(5) // duff
  });
  ...
  self.CURRENCY_REGEX = new RegExp('\\b(duff?|NXS|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)
...
// return via callback amount in NXS, and if it was needed to convertion rate and originalCurrency
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
  ...
  if (inputValue === 'all' && user !== undefined) {
    user.getBalance(user.id, 6, function (err, balance) {
      ...
      let value = coin.toDuff(balance);
      debug('Log: using ALL balance of ' + user.name + ' = ' + balance);
      cb(value, null, null, null, coin.toCoin(value) + ' NXS');
    });
    ...
    } else if (unit.match(/NXS/i)) {
      currency = 'NXS';
      value = coin.toDuff(inputValue);
    } else {
    ...
    self.getPriceRates(function (err, rates) {
      ...
      if (!rate) {
        ...
        let text = value.toFixed(2) + ' ' + currency + ' ' +
          '(' + coin.toCoin(newValue) + ' NXS at ' + rate.toFixed(2) + ' ' + currency + ' / NXS)';
        // return converted value in NXS,  convertion rate, originalCurrency, originalValue, text about the convertion
        return cb(newValue, rate, currency, value, text);
      }
    });
  } else {
    // amount is in NXS, return only value, no convertion rate
    return cb(value, null, null, null, coin.toCoin(value) + ' NXS');
  }
};
                ...
                self.getDirectMessageChannelID(null, oneUser.id, function (err, DMchannelRecievingUser) {
                  if (err === null) {
                    ...
                    let recievingUserMessage = {
                      ...
                      'text': texts.SunRecieved + coin.toCoin(sunraySize) + ' NXS'
                    };
  ...
  self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
    ...
    if (message.match(/\bwithdraw\b/i)) {
      ...
      if (providedCurrency !== null && providedCurrency[0].length !== 0) {
        ...
        //not provided, set NXS as default currency
        amount[2] = 'NXS';
      }
      debug(amount);
      ...
      // convert amount if currency isn't NXS
        ...
        if (fromCurrency === 'NXS') {
          ...
          self.normalizeValue(amount[1], toCurrency, user, function (value, rate) {
            if (rate === false) {
              ...
              reply.text = amount[1] + ' ' + fromCurrency + ' = ' + newValue.toPrecision(4) + '  ' + toCurrency +
                ' ( 1.0 NXS = ' + rate.toPrecision(4) + ' ' + toCurrency + ' )';
            }
            self.slack.say(reply);
          });
        } else if (toCurrency === 'NXS') {
          // other -> NXS
          self.normalizeValue(amount[1], fromCurrency, user, function (value, rate) {
            if (rate === false) {
              ...
              reply.text = amount[1] + ' ' + fromCurrency + ' = ' + coin.toCoin(value) + '  ' + toCurrency +
                ' ( 1.0 ' + fromCurrency + ' = ' + (1.0 / rate).toPrecision(4) + ' NXS)';
            }
            self.slack.say(reply);
          });
        }
      ...
      sun.getSunBalance(self.sunUser, function (err, sunBalance) {
        if (err) {
          ...
          if (sunBalance !== undefined && sunBalance > 2e-8) {
            reply.text = texts.SunAvailibleAmount + sunBalance + ' NXS';
          } else {
      ...
      if (user.is_admin) {
        ...
        if (message.match(/\bthreshold\b/i)) {
          ...
          if (amount !== null) {
            ...
            sun.saveThreshold(coin.toDuff(amount[1]),
              function (err) {
                if (err) {
                  debug(err); return;
                } else {
    ...
    //  * QUIZ
    if (self.OPTIONS.ENABLE_QUIZ_FEATURE && message.match(/\bquiz\b/i)) {
      ...
      if (user.is_admin) {
        ...
        if (message.match(/\bstart\b/i)) {
          let amount = message.match(/\d+/i);
          if (amount === null) {
            privateReply.text = texts.QuizStartNoAmount;
            self.slack.say(privateReply);
            return;
          }

          quiz.start(amount, function (err) {
            if (err) {
