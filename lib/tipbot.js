...
var TipBot = function(slack, TESTNET, OPTIONS) {
    var self = this;
...
// TODO: currencies / Dash
    ...
    self.AMOUNT_REGEX = new RegExp("(\\d+\\.?\\d*) *(Satoshis?|DASH|" + self.CURRENCIES.join("|") + ")", "i");
...
// TODO ?? don't need, only for blocktrail online wallet ?
        ...
        self.getPriceRates(function(err, rates) {
            ...
            if (!rate) {
                ...
                var text = value.toFixed(2) + " " + currency + " " +
                    "(" + blocktrail.toBTC(newValue) + " DASH at " + rate.rate.toFixed(2) + " " + currency + " / DASH)";
...
TipBot.prototype.tellHelp = function(channel) {
    channel.send(
        "*TIPBOT COMMANDS* \n" +
        "*!! STILL IN DEVELOPMENT, USING A SEPERATED WALLET !! \n\n" +
        " - *balance*\t\task the bot for your current balance; Answers only if asked in a Direct Message: _/DM @tipbot what is my balance_ \n" +
        ...
        " - *withdraw*\ttell the bot to withdraw to a address; _@tipbot withdraw 1 DASH to 1dice8EMZmqKvrGE4Qc9bUFf9PX3xaYDp!_ \n" +
        ...
        " - *convert*\task the bot to convert between a particular currency and Dash (or visa versa); _@tipbot 1 USD to EUR!_ \n" +
        "\t\t\t\t\t\t\tor; _@tipbot 0.03 DASH to GBP_ \n"
    );
};
...
// TODO CHECK IF ADDRESS IS A VALID DASH ADDRESS
        if (address) {
 ...
 // TODO CHECK IF ADDRESS IS A VALID DASH ADDRESS 
                ...
                //     channel.send("Sorry " + user.handle + " that's not a Dash address!");
                        ...
                        if (rate === false) {
                            ...
                            self.getPriceRates(function(err, rates) {
                                ...
                                if (!rate) {
                                    ...
                                    channel.send(newValue.toFixed(2) + " " + currency + " (" + rate.rate.toFixed(2) + " " + currency + " / DASH)");
                                }
                            });
                        }
                    });
        ...
        if (currency) {
            ...
            self.getPriceRates(function(err, rates) {
                ...
                if (!rate) {
                    ...
                    channel.send("1.0 DASH is " + rate.rate.toFixed(2) + " " + currency);
                }
            });
