...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    if (message.match(/\brain\b/i)) {
        ...
        if (self.OPTIONS.RAIN_TIMER !== undefined) {
            ...
            reply["text"] += self.text["RainTimer"] +
                self.OPTIONS.RAIN_TIMER / 1000 / 60 +
...
function getRainBalance(self, user, cb) {
    if (self.RainUser === undefined || self.RainUser === null) {
        debug("ERROR cannot get rain balance as Rain User is unknown !");
...
// returns raindrop size, amountOnlineUsers
function getCurrentRainDropsSize(self, user, cb) {
    listOnlineUsers(self, function (err, onlineUsers) {
        ...
        getRainBalance(self, user, function (err, rainBalance) {
            if (err) {
                debug("ERROR cannnot calculated size of a raindrop if available rain is unknow !");
...
function rainNow(self, user, channel) {
    if (self.RainUser === undefined || self.RainUser === null) {
        debug("ERROR cannot let is rain as Rain User is unknown !");
        return;
    }
    // get raindrop size
    getCurrentRainDropsSize(self, user, function (err, rainDropSize) {
        if (err) {
            debug("ERROR cannot make it rain as raindrop size is unknown !");
            return;
        } else {
            // get online users 
            listOnlineUsers(self, function (err, onlineUsers) {
                if (!err) {
                    ...
                    reply["text"] += self.text["RainNow"] + rainDropSize.toPrecision(4) + " dash.\n";
                    reply["text"] += self.text["RainReqDonation_1"] + self.OPTIONS.RAIN_USER + self.text["RainReqDonation_2"] + "\n";
                    reply["text"] += self.text["RainClouds"] + "\n";
                    self.slack.say(reply);
                    ...
                    for (var getWet in onlineUsers) {
                        ...
                        debug("A raindrop of " +
                            rainDropSize.toPrecision(4) + " dash falls on " +
                            getWet);
                            
// todo: special move function in user.js because normal send doesn't work (spam protection / locks)
                       // self.RainUser.send(channel.id, getWet, rainDropSize);
                    }
                }
            });
        }
