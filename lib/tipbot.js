...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    if (message.match(/\brain\b/i)) {
        ...
        self.slack.api.users.list({ "presence": 1 }, function (err, data) {
            ...
            // reply["text"] = "Amount of online users: " + amountOnlineUsers;
            // self.slack.say(reply);
            ...
            if (rainUser === null) {
                ...
                user.getBalance(rainUser, 6, function (err, rainBalance) {
                    if (err) return;
                    reply["text"] = "Availible rain amount = " + rainBalance + " dash" +
                        " == " + rainBalance / amountOnlineUsers + " dash / online user";
                    self.slack.say(reply);
                });
            }
        });
