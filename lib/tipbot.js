        ...
        self.slack.api.users.list({ "presence": 1 }, function (err, data) {
            ...
            if (findRainUser === undefined || findRainUser.length !== 1) {
                ...
                user.getBalance(findRainUserID, 6, function (err, rainBalance) {
                    if (err) return;
                    // rain / online user
                    var rain = rainBalance / amountOnlineUsers;
                    reply["text"] = "Available rain : " + rainBalance + " dash \n" +
                        rain.toPrecision(4) + " rain drops for each online user (" + amountOnlineUsers + ")";
                    self.slack.say(reply);

                    if (message.match(/\bnow\b/i)) {
                        // let it rain NOW
                        for (var getWet in onlineUsers) {
                            debug("Give " + getWet + " " + rain.toPrecision(4) + " from " + findRainUser[0].name);
