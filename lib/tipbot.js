...
TipBot.prototype.showRandomHelp = function () {
    ...
    var helpCount = self.text["helpText"].length;
    var getHelpNR = Math.floor((Math.random() * helpCount)); 
    getHelpNR = getHelpNR == 0 ? 1 : getHelpNR;                         // don't show title
    var helpTxt = self.text["helpText"][getHelpNR].replace(/[*]/g,"").replace(/[_]/g,"");     // * for bold doesn't show in a code block
...
TipBot.prototype.sunShineNow = function () {
    ...
    getSunBalance(self, function (err, sunBalance) {
        ...
        if (sunBalance !== undefined) {
            var reply = { "channel": self.OPTIONS.MAIN_CHANNEL.id };
            if (err) {
