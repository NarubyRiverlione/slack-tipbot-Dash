...
TipBot.prototype.getDirectMessageChannelID = function (channel, userID, cb) {
    ...
    var firstCharOfChannelID = channel.substring(0, 1);
    if (firstCharOfChannelID === "D") return channel;
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    if (message.match(/(send|give|sent|tip)/i)) {
        ...
        if (userMatches.length == 0) {
            reply["text"] = "Hello there " + user.name + ", I didn't catch the user you want to tip. You surrly didn't want to tip yourself now did you ?";
            self.slack.say(reply);
    ...
    if (message.match(/currencies/i)) {
        reply["text"] = "Price info from coinmarketcap \n" +
            ...
            "and does it needs saying: *DASH* is supported !";
        self.slack.say(reply);
        return;
    }
