...
TipBot.prototype.tellPriceList = function (priceChannel) {
    var self = this;
    var reply = { "channel": priceChannel };
    reply["text"] = "Price information:";
    self.slack.say(reply);

    for (var currency in self.LIST_PRICES) {
        debug("Pricelist: show " + self.LIST_PRICES[currency]);
        tellPrice(self, reply, self.LIST_PRICES[currency].toLowerCase());
    }
};
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    //     * PRICE LIST
    if (message.match(/\bpricelist\b/i)) {
        self.tellPriceList(self.OPTIONS.PRICE_CHANNEL.id);
        return;
    }
    ...
    if (message.match(/\brain\b/i)) {
        ...
        self.slack.api.users.list({ "presence": 1 }, function (err, data) {
           ...
           // get Rain User
            var rainUser = _.filter(self.users, function (match) { return match.name.match({self.OPTIONS.RAIN_USER, "i" }) });
