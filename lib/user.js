...
// TODO: DRY
            ...
            if (unconfirmedBalance === balance) {
                ...
                cb(null, self.handle + " your tip jar contains:: " + balance + " Dash, \n" +
                    "you have an unconfirmed balance (less than " + self.REQUIRED_CONFIRMATIONS + " confirmations) of " + unconfirmedBalance + " DASH");
            }
        });
    });
};
 ...
 //   self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance, resHeaders) {
    self.getBalance(self.id,self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (balance >= value + self.tx_fee) {
            ...
            reply["text"] = "Sorry " + self.handle + " insufficient balance.";
            slack.say(reply);
        }
    });
};
