    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        self.getBalance(self.id, 1, function (err, unconfirmedBalance) {
            ...
            if (unconfirmedBalance === balance) {
                cb(null, self.handle + self.text["BalanceIs"] + balance + " " + self.text["BaseCurrency"]);
            } else {
                cb(null, self.handle + self.text["BalanceIs"] + balance + " Dash, \n" +
            ...
            if (result === true) {
                ...
                self.tipbot.getDirectMessageChannelID(null, sendToUser.id, function (err, DMchannelRecievingUser) {
                    if (err === null) {
                        ...
                        recievingUserMessage["text"] = self.handle + self.text["SendPrivateMssRecievingUser_1"] + blocktrail.toBTC(value) + " " + self.text["BaseCurrency"] + self.text["SendPrivateMssRecievingUser_2"] + self.handle + " !";
