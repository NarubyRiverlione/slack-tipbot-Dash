...
let helpText = require('../text/dash.js').userTxt;
...
function sendAmount(self, toAddress, value, cb) {
    ...
    self.wallet.sendFrom(self.id, toAddress, parseFloat(toDash(value)),
        function (err, tx_id) {
            if (err) {
            ...
            let line = helpText.Withdrawal1 + toDash(value) + ' ' +
...
User.prototype.getBalanceLine = function (cb) {
    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (toDuff(balance) >= self.tipbot.HighBalanceWarningMark) {
            // warn user
...
User.prototype.send = function (sendToUser, value, cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        self.wallet.move(self.id, sendToUser.id, parseFloat(toDash(value)), function (err, result) {
            if (err) {
                debug('ERROR: moving between account (' + self.id + ') to acount (' + sendToUser.id + ')' + err);
                error = helpText.SendOops1 + toDash(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.name + '';
                self.locked = false; //  this transaction is done, clear lock to allow new transactions
                cb(error, null);
            ...
            if (result === true) {
                ...
                debug('Sending Tip: Moved ' + toDash(value) + ' Dash from ' + self.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
                ...
                responses.privateToReciever = helpText.SendPrivateMssRecievingUser1 + sendToUser.handle +
                    helpText.SendPrivateMssRecievingUser2 + toDash(value) + ' ' +
