...
User.prototype.getBalanceLine = function (cb) {
  ...
  this.getBalance(this.id, this.REQUIRED_WITHDRAW_CONFIRMATIONS,
    (err, balance) => {
      ...
      if (dash.toDuff(balance) >= this.tipbot.HighBalanceWarningMark) {
        // warn user
...
User.prototype.send = function (sendToUser, value, cb) {
  ...
  this.wallet.getBalance(this.id, this.REQUIRED_TIP_CONFIRMATIONS,
    (err, balance) => {
      ...
      this.wallet.move(this.id, sendToUser.id, parseFloat(dash.toDash(value)),
        (err, result) => {
          if (err) {
          ...
          if (result === true) {
            ...
            debug('Sending Tip: Moved ' + dash.toDash(value) + ' Dash from ' + this.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
