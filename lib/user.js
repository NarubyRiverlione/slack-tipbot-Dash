...
let User = function (tipbot, userId, userName, isAdmin) {
  ...
  this.blockchainUrl = 'https://chainz.cryptoid.info/dash/tx.dws?';

  this.REQUIRED_WITHDRAW_CONFIRMATIONS = 6;
  this.REQUIRED_TIP_CONFIRMATIONS = 5; // to immediately be able to send a tip after deposit with InstantSend
...
function sendAmount(self, toAddress, value, cb) {
  ...
  self.wallet.sendFrom(self.id, toAddress, parseFloat(Coin.toLarge(value)),
    (err, tx_id) => {
      if (err) {
...
User.prototype.getBalanceLine = function (cb) {
  ...
  this.getBalance(this.id, this.REQUIRED_WITHDRAW_CONFIRMATIONS,
    (err, balance) => {
      ...
      if (Coin.toSmall(balance) >= this.tipbot.HighBalanceWarningMark) {
        // warn user
      ...
      this.getBalance(this.id, 1,
        (err, unconfirmedBalance) => {
          ...
          if (unconfirmedBalance !== balance) {
            ...
            balanceText += '\n' + helpText.UnconfirmedBalance1 + this.REQUIRED_WITHDRAW_CONFIRMATIONS + helpText.UnconfirmedBalance2 + unconfirmedBalance + ' ' + helpText.BaseCurrency;
          }
...
User.prototype.send = function (sendToUser, value, cb) {
  ...
  this.wallet.getBalance(this.id, this.REQUIRED_TIP_CONFIRMATIONS,
    (err, balance) => {
      balance = Coin.toSmall(balance);
      if (err) {
      ...
      this.wallet.move(this.id, sendToUser.id, parseFloat(Coin.toLarge(value)),
        (err, result) => {
          if (err) {
          ...
          if (result === true) {
            ...
            debug('Sending Tip: Moved ' + Coin.toLarge(value) + ' Dash from ' + this.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
