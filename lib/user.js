...
User.prototype.send = function (channelID, sendToUser, value,cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        /* don't need the check, if account doesn't exits then move wil create one
                // check if reciever has an address in the wallet
                // (maybe he has got an account yet, if so create one now)
        ...
        */
        ...
        setTimeout(function () {
            self.locked = false;
            reply["text"] = "--- send " + value +
                " from " + self.handle +
                " to " + sendToUser.getSlackHandle();
