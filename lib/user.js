...
function sendAmount(self, toAddress, value, cb) {
    ...
    self.wallet.sendFrom(self.id, toAddress, parseFloat(blocktrail.toBTC(value)),
        function (err, tx_id) {
            ...
            let line = self.text.Withdrawal1 + blocktrail.toBTC(value) + ' ' +
                self.text.BaseCurrency + ' to ' +
                toAddress +
                self.text.WithdrawalTransaction +
                url;
            cb(null, line);
    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        self.getBalance(self.id, 1, function (err, unconfirmedBalance) {
            ...
            if (unconfirmedBalance !== balance) {
                ...
                balanceText += '\n' + self.text.UnconfirmedBalance1 + self.REQUIRED_CONFIRMATIONS + self.text.UnconfirmedBalance2 + unconfirmedBalance + ' ' + self.text.BaseCurrency;
            }
            cb(null, balanceText);
        });
    });
...
User.prototype.withdraw = function (value, toAddress, walletPass, cb) {
    ...
    if (self.locked === true) {
        let error = self.handle + self.text.Locked;
        cb(error, null);
        return;
    }
    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (balance >= value + self.tipbot.OPTIONS.TX_FEE) {
            ...
            // not enough balance
            let error = self.text.InsufficientBalance1 + self.handle + self.text.InsufficientBalance2;
            cb(error, null);
        }
...
User.prototype.send = function (sendToUser, value, cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (balance < value) {
            error = self.text.InsufficientBalance1 + self.handle + self.text.InsufficientBalance2;
            self.locked = false; //  this transaction is done, clear lock to allow new transactions
            cb(error, null);
            return;
        }
        ...
        self.wallet.move(self.id, sendToUser.id, parseFloat(blocktrail.toBTC(value)), function (err, result) {
            if (err) {
                ...
                error = self.text.SendOops1 + blocktrail.toBTC(value) + ' ' + self.text.BaseCurrency + self.text.SendOops2 + sendToUser.name + '';
                self.locked = false; //  this transaction is done, clear lock to allow new transactions
                cb(error, null);
            ...
            if (result === true) {
                ...
                debug('Sending Tip: Moved ' + blocktrail.toBTC(value) + ' Dash from ' + self.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
                // prepare message in channel where Tip command was issued
                let responses = {
                    public: self.text.SendPublicMessage1 + self.handle + self.text.SendPublicMessage2 + sendToUser.handle
                };
                //  prepare message to recieving user to inform of tip
                responses.privateToReciever = self.text.SendPrivateMssRecievingUser1 + sendToUser.handle +
                    self.text.SendPrivateMssRecievingUser2 + blocktrail.toBTC(value) + ' ' +
                    self.text.BaseCurrency + self.text.SendPrivateMssRecievingUser3 + self.handle + ' !';
