...
User.prototype.getBalanceLine = function (cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance, resHeaders) {
        if (err) {
        ...
        self.wallet.getBalance(self.id, 1, function (err, unconfirmedBalance, resHeaders) {
            ...
            if (unconfirmedBalance === balance) {
                cb(null, self.handle + " your tip jar contains: " + balance + " Dash");
            } else {
                cb(null, self.handle + " your tip jar contains:: " + balance + " Dash, \n" +
                    "you have an unconfirmed balance (less then " + self.REQUIRED_CONFIRMATIONS + " confirmations) of " + unconfirmedBalance + " DASH");
            }
        });
    });
};
        ...
        if (balance >= value + self.tx_fee) {
            ...
            self.wallet.sendFrom(self.id, toAddress, parseFloat(blocktrail.toBTC(value)),
                function (err, tx_id, resHeaders) {
                    ...
                    reply["text"] = "Withdrawal of " + blocktrail.toBTC(value) + " Dash  to "
                        + toAddress
                        + " transaction: " + url;
                    slack.say(reply);
...
User.prototype.send = function (channelID, sendToUser, value) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance, resHeaders) {
        ...
        if (balance < value) {
            reply["text"] = "Sorry " + self.handle + " you do not have enough (confirmed) balance to do this ...";
            slack.say(reply);
            return;
        }
...
// don't repeat ammount, send it in a DM to recieving user
                //   reply["text"] = "Sent " + blocktrail.toBTC(value) + " Dash from " + self.name + " to " + sendToUser.name;
 //TODO add emo great / smile   
                reply["text"] =  ":clap: " + self.handle + " tipped " + sendToUser.handle;
                slack.say(reply);
                ...
                self.tipbot.getDirectMessageChannelID(null, self.id, function (err, DMchannelSendingUser) {
                    if (err === null) {
                        ...
                        self.getBalanceLine(function (err, response) {
                            sendingUserMessage["text"] = self.handle + " you're a great Dash Chat user :thumbsup: \n" +
                ...
                self.tipbot.getDirectMessageChannelID(null, sendToUser.id, function (err, DMchannelRecievingUser) {
                   ...
                   //TODO add message that was used to send      
                        recievingUserMessage["text"] = "Hi there! You just received " + blocktrail.toBTC(value) + " Dash from " + self.handle + " !" ;
