'use strict';

let debug = require('debug')('tipbot:user');

let helpText = require('../text/txt_nexus.js').userTxt;
let coin = require('./coin');


let User = function (tipbot, userId, userName, isAdmin) {
  this.tipbot = tipbot;
  this.id = userId;
  this.name = userName;
  this.is_admin = isAdmin || false;
  this.handle = this.getSlackHandle();

  this.wallet = tipbot.wallet;// RPC connection to wallet
  this.blockchainUrl = 'http://nexusoft.io/platform/client/tx.php?hash=';

  this.REQUIRED_CONFIRMATIONS = 6;

  this.locked = false;  // to prevent multiple transactions
};

// send via Blockchain
function sendAmount(self, toAddress, value, cb) {
  // send  users Account to Address
  self.wallet.sendFrom(self.id, toAddress, parseFloat(coin.toCoin(value)),
    (err, tx_id) => {
      if (err) {
        debug('ERROR sending via blockchain : ' + err);
        let error = 'An error prevents withdrawing.';
        cb(error, null);
        return;
      }
      let url = self.blockchainUrl + tx_id;
      let line = helpText.Withdrawal1 + coin.toCoin(value) + ' ' +
        helpText.BaseCurrency + ' to ' +
        toAddress +
        helpText.WithdrawalTransaction +
        url;
      cb(null, line);

    });
}

User.prototype.updateFromMember = function (member) {
  //   let self = this;
  this.name = member.name;
  this.is_admin = member.is_admin;
  this.handle = this.getSlackHandle();
};


User.prototype.getBalanceLine = function (cb) {
  // let self = this;
  let balanceText = '';

  this.getBalance(this.id, this.REQUIRED_CONFIRMATIONS,
    (err, balance) => {
      if (err) { cb(err, null); return; }
      if (balance === 0) { cb(null, helpText.NoBalance); return; }
      balanceText = this.handle + helpText.BalanceIs + balance + ' ' + helpText.BaseCurrency;
      // check for High Balance
      if (coin.toDuff(balance) >= this.tipbot.HighBalanceWarningMark) {
        // warn user
        balanceText += '\n *' + helpText.BalanceWarningHigh + '*';
      }
      // check if there is an unconfirmed balance
      this.getBalance(this.id, 1,
        (err, unconfirmedBalance) => {
          if (err) { cb(err, null); return; }
          if (unconfirmedBalance !== balance) {
            // add unconfirmed balance information to the text
            balanceText += '\n' + helpText.UnconfirmedBalance1 + this.REQUIRED_CONFIRMATIONS + helpText.UnconfirmedBalance2 + unconfirmedBalance + ' ' + helpText.BaseCurrency;
          }
          debug(balanceText + ' (' + this.id + ')');
          cb(null, balanceText);
        });
    });
};

// reusable getBalance function
User.prototype.getBalance = function (userID, reqConfirmantions, cb) {
  // let self = this;
  // debug('Get balance with ' + reqConfirmantions + ' confirmations for ' + userID);
  this.wallet.getBalance(userID, reqConfirmantions,
    (err, balance) => {
      if (err) {
        let errorTx = 'ERROR getting balance with ' + reqConfirmantions + ' confirmations: ' + err;
        debug(errorTx);
        cb(errorTx, null);
      } else {
        cb(null, balance);
      }
    });
};

User.prototype.tellDepositeAddress = function (cb) {
  //   let self = this;
  let depositAddress;
  // get all addresses in de the wallet for this user
  // should be 1 if users has already an account, if he hasn't give hem one now
  this.wallet.getAddressesByAccount(this.id,
    (err, addresses) => {
      if (err) { cb(err, null); }

      if (addresses !== undefined && addresses.length > 0) {
        // found an address for this userID in the wallet
        depositAddress = addresses[0]; // get first address
        debug('Existing address for ' + this.handle + '  ' + depositAddress);
        cb(null, this.handle + ' you can deposit to: ' + depositAddress);
      } else {
        // didn't find an address for this user in the wallet, create an account now
        this.wallet.getNewAddress(this.id,
          (err, address) => {
            if (err) {
              cb(err, null);
            }

            depositAddress = address;
            debug('New address for ' + this.handle + '  ' + depositAddress);
            cb(null, this.handle + ' you can deposit to: ' + depositAddress);
          });
      }
    });
};

User.prototype.withdraw = function (value, toAddress, walletPass, cb) {
  // value is in satochi !
  //  let self = this;
  // prevent multiple transactions, only continue if not already locked
  if (this.locked === true) {
    let error = this.handle + helpText.Locked;
    cb(error, null);
    return;
  }

  // lock now to prevent new transactions
  this.locked = true;

  this.getBalance(this.id, this.REQUIRED_CONFIRMATIONS,
    (err, balance) => {
      if (err) {
        this.locked = false;
        cb(err, null);
        return;
      }
      balance = coin.toDuff(balance);

      // check if with all (value = balance)
      if (value === balance) {
        // substract tx_fee from value
        value = balance - this.tipbot.OPTIONS.TX_FEE;
      }

      if (balance >= value + this.tipbot.OPTIONS.TX_FEE) {
        // enough balance
        if (value === balance) {
          // withdraw everything (minus the fee)
          value -= this.tipbot.OPTIONS.TX_FEE;
        }
        // unlock wallet if needed
        if (walletPass) {
          this.wallet.walletPassphrase(walletPass, 10,
            err => {
              if (err) {
                let error = 'ERROR could not unlock the wallet';
                debug(error + ' : ' + err);
                this.locked = false;
                cb(error, null);

                return;
              }
              // wallet is now unlocked for 10 seconds, move amount
              sendAmount(this, toAddress, value, cb);
            });
        } else {

          // no wallet unlocking needed, no need to wait for the  walletPassphrase callback
          sendAmount(this, toAddress, value, cb);

        }
      } else {
        // not enough balance
        let error = helpText.InsufficientBalance1 + this.handle + helpText.InsufficientBalance2;
        cb(error, null);
      }

      //  this transaction is done, clear lock to allow new transactions

      this.locked = false;
    });
};

User.prototype.send = function (sendToUser, value, cb) {
  //   let self = this;

  let error = '';
  // prevent multiple transactions, only continue if not already locked
  if (this.locked === true) {
    error = this.handle + helpText.Locked;
    cb(error, null);
    return;

  }
  // lock now to prevent new transactions

  this.locked = true;

  // check balance before sending amount to prevent negative saldo
  this.wallet.getBalance(this.id, this.REQUIRED_CONFIRMATIONS,
    (err, balance) => {
      balance = coin.toDuff(balance);
      if (err) {
        debug('ERROR checking balance before sending tip: ' + err);
        let error = 'ERROR checking balance before sending tip.';
        this.locked = false; //  this transaction is done, clear lock to allow new transactions
        cb(error, null);
        return;
      }
      if (balance < value) {
        error = helpText.InsufficientBalance1 + this.handle + helpText.InsufficientBalance2;
        this.locked = false; //  this transaction is done, clear lock to allow new transactions
        cb(error, null);
        return;
      }

      // use a in wallet transfer
      this.wallet.move(this.id, sendToUser.id, parseFloat(coin.toCoin(value)),
        (err, result) => {
          if (err) {
            debug('ERROR: moving between account (' + this.id + ') to acount (' + sendToUser.id + ')' + err);
            error = helpText.SendOops1 + coin.toCoin(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.name + '';
            this.locked = false; //  this transaction is done, clear lock to allow new transactions
            cb(error, null);

            return;
          }

          if (result === true) {

            debug('Sending Tip: Moved ' + coin.toCoin(value) + ' NXS from ' + this.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
            // prepare message in channel where Tip command was issued
            let responses = {
              public: helpText.SendPublicMessage1 + this.handle + helpText.SendPublicMessage2 + sendToUser.handle
            };
            //  prepare message to recieving user to inform of tip
            responses.privateToReciever = helpText.SendPrivateMssRecievingUser1 + sendToUser.handle +
              helpText.SendPrivateMssRecievingUser2 + coin.toCoin(value) + ' ' +
              helpText.BaseCurrency + helpText.SendPrivateMssRecievingUser3 + this.handle + ' !';
            // prepare message to sending user to inform of new balance
            this.getBalanceLine(
              (err, balanceLine) => {
                responses.privateToSender = this.handle + helpText.SendPrivateMssSendingUser + balanceLine;
                this.locked = false; //  this transaction is done, clear lock to allow new transactions
                // all responses are prepared, send them in tipbot.js
                cb(null, responses);
              });
          } else {
            // result == false
            error = helpText.SendOops1 + coin.toCoin(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.name + '';
            this.locked = false;  //  this transaction is done, clear lock to allow new transactions
            cb(error, null);
          }

        });
    });

};

User.prototype.validateAddress = function (address, cb) {
  this.wallet.validateAddress(address, (err, info) => {
    if (err) {
      debug('Error validating address "' + address + '" : ' + err);
      cb(err, null);
    } else {
      cb(null, info);
    }
  });
};

User.prototype.getSlackHandle = function () {
  // let self = this;
  return '<@' + this.id + '|' + this.name + '>';
};

User.fromMember = function (tipbot, member) {
  let user = new User(tipbot, member.id, member.name, member.is_admin);
  return user;
};

module.exports = User;
